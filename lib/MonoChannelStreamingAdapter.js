...
const StreamingSessionState = require('./MonoChannelStreamingSessionState');
...
const logger = require('pino')({ 
    level: LOG_LEVEL,
    name: 'MonoChannelStreamingAdapter'
});
...
function sendOpenedResponse (webSocket, openRequest, sessionState){
  ...
  // Send the opened message back to the client 
...
function setActiveSpeechEngine(speechToTextEngine, webSocket, openRequest, sessionState) {
  ...
  if (speechToTextEngine != null){
    speechToTextEngine.on('listening', () => {
      ...
      sessionState.setSpeechEngineListening('monoSpeechToTextEnding', true);
  
      if (sessionState.getPreListenCache('monoPreListenCache') != null){
        logger.debug('Writing monoPreListenCache to speech engine. Number of bytes: ' + sessionState.getPreListenCache('monoPreListenCache').length);
        speechToTextEngine.write(sessionState.getPreListenCache('monoPreListenCache'));
        sessionState.setPreListenCache('monoPreListenCache',null);
      }
    });
 ...
 */
function handleMonoChannelStreamingConnection(webSocket, incomingMessage) {
  ...
  let monoSpeechToTextEngine = null;
  ...
  webSocket.on('message', (data) => {
     if (typeof data === 'string') {
        try {
          ...
          if (sessionState != null){
            ...
            if (message.seq != sessionState.clientSeq){
              logger.debug(data.length, 'FATAL PROTOCOL ERROR: Received command message from websocket connection with unexpexted sequence number; seq received: ' + message.seq + 'seq expected: ' + sessionState.clientSeq);  
            }
          }
          ...
          if (message.type === "open") {
            ...
            if (message.parameters.media[0].channels[0] != 'mono-mixed'){
              logger.error("ERROR: this adapter only supports mono-mixed. Closing websocket");
              webSocket.close();
              return;
            }
            ...
            if (message.parameters.conversationId !== "00000000-0000-0000-0000-000000000000"){
              ...
              //  audio streams.
              monoSpeechToTextEngine = new SpeechToTextEngine();
              
              setActiveSpeechEngine(monoSpeechToTextEngine, webSocket, message, sessionState);          
            }
            else{
              //  Probe open received. Go ahead and immediately respond.
          ...
          if (message.type === "close") {
            ...
            if (monoSpeechToTextEngine != null){
              removeActiveSpeechEngines(monoSpeechToTextEngine, sessionState);
              monoSpeechToTextEngine = null;
            }
      ...
      if (monoSpeechToTextEngine != null && sessionState.isSpeechEngineListening('monoSpeechToTextEngine') == true) {
        if (sessionState.receivedBufferCount % 100 == 0)
          logger.trace('Writing ' + externalBuffer.length + ' bytes to external speech engine.');  

        monoSpeechToTextEngine.write(channelBuffer);
      }
      else{
        //  Here we cache the audio until the internal speech engine is listening and ready for the data.
        if (sessionState.getPreListenCache('monoPreListenCache') == null){
          sessionState.setPreListenCache('monoPreListenCache',channelBuffer);
        }
        else{
          //  Here we cache the audio until the external speech engine is listening and ready for the data.
          let preListenCache = sessionState.getPreListenCache('monoPreListenCache');
          ...
          sessionState.setPreListenCache('monoPreListenCache',newBuffer);
        }
      }
    } 
    else {
...
module.exports.handleMonoChannelStreamingConnection = handleMonoChannelStreamingConnection;